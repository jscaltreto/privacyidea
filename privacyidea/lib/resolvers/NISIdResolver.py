...
"""
  Description:  This file is part of the privacyidea service
                ...
                for resolving user info via NIS maps
...
class IdResolver (UserIdResolver):
    ...
    def setup(config=None, cache_dir=None):
        ...
        """
        log.info("Setting up the NISResolver")
        return
    ...
    def loadMap(self):
        ...
        """
        Loads the data from the NIS domain.
        if the self.domain is empty, it loads the system defaultdomain.
        Empty lines are ignored.
        """

        if self.domain == "":
            self.domain = nis.get_default_domain()

        log.info('loading users from NIS Domain %s' % self.domain)
        
        try:
            users = nis.cat('passwd', self.domain)
        except nis.error as msg:
            log.error('Unable to load users from NIS: %s' % msg)
            return
    ...
    def checkPass(self, uid, password):
        ...
        of nis map needs to contain the crypted password
...
#############################################################
    ...
    def getResolverClassDescriptor(cls):
        ...
        descriptor['clazz'] = "useridresolver.NISIdResolver.IdResolver"
